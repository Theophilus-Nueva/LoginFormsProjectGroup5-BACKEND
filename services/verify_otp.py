...
def validate_and_consume_otp(user_id: str, otp_code: str, request: Request = None) -> dict:
    ...
    try:
        ...
        if not valid_otp:
             log_auth_event(email="OTP Validation", event_type="MFA_FAILED_INVALID_CODE", user_id=user_id, request=request)
        ...
        log_auth_event(email="User Verified", event_type="MFA_SUCCESS", user_id=user_id, request=request)
